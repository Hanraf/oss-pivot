...
export default function MonthlyTarget() {
  ...
  const permohonanDiprosesCount = datas.filter((p) => p.id_progress === 2 ).length;
  const permohonanSelesaiCount = datas.filter((p) => p.id_progress !== 2 ).length;
  const totalPermohonan = permohonanDiprosesCount + permohonanSelesaiCount;
  const permohonanPersenCount = totalPermohonan > 0 ? (permohonanSelesaiCount / totalPermohonan) * 100 : 0;
  
  const series = [permohonanPersenCount];
        ...
        <div className="flex justify-between">
          <div>
            ...
            <p className="mt-1 text-gray-500 text-theme-sm dark:text-gray-400">
              Tugas selesai dari seluruh permohonan
            </p>
          </div>
